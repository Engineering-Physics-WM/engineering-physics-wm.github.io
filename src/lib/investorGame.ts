...
import type { InvestorGameActivityRow, InvestorGamePlayerRow, ProjectId } from "../types/domain";
...
export const describeChanges = (
  ...
  const from = normalizeAllocations(before, projectIds);
  const to = normalizeAllocations(after, projectIds);
  return projectIds
...
export const fetchActivity = async (gameId: string, limit = 2000) => {
  ...
  const { data, error } = await supabase
    ...
    .select(
      "id, created_at, player_id, event_label, allocations_before, allocations_after, total_before, total_after"
    )
    .eq("game_id", gameId)
    .order("created_at", { ascending: false })
    .limit(limit);
  return { rows: (data ?? []) as unknown as AdminActivity[], error };
};
...
export const deletePlayer = async (playerId: string) => {
  ...
  const { data, error } = await supabase
    ...
    .select("id");
  if (error) return { error };
  if (!data?.length) return { error: { message: "That student was not removed." } };
  return { error: null };
};
