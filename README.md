...
Public cohort updates load from Supabase `cohort_announcements` when configured. The checked-in `js/data.js` `announcements2026` array remains the fallback and seed source. Files for announcement links can be placed in `public/announcements/<year>/` and linked as `/announcements/<year>/filename.pdf`. Editing the fallback file still requires commit/push/deploy; dashboard-posted announcements update the live public feed directly.

The protected dashboard has an **Email drafts** tab that can turn a news item or custom note into a `mailto:` draft for selected groups: all students + mentors, all students, Honors students, all mentors, selected team students + mentors, selected team students, or selected team mentors. Student recipients come from the private Supabase allowlist or saved teams, never from a checked-in public roster. This opens the default mail app with BCC recipients filled in; the instructor still reviews and sends the message manually. A custom draft can also be posted as the public **Now** news item; the app strips the email greeting/signature, uses the subject as the title, and formats the remaining text as a news summary/body.
...
For the Supabase-backed announcement system, run `supabase/announcements-schema.sql`. Public published announcements can be read by anyone; draft/edit/send-job access stays limited to `rxyan2@wm.edu`. Use `event_date` for the date shown in the news UI and `publish_at` for when the row becomes public. Actual email delivery still happens through the instructor-reviewed Mail draft flow, not automatically from the site.
