...
const DEFAULT_MATCHING_MODE: MatchingModeId = "top3";
const MATCHING_MODE_OPTIONS: MatchingModeOption[] = [
  {
    id: "top3",
    label: "Top 3 balance",
...
type DemandMap = {
  top3Demand: Record<ProjectId, number>;
  top1Demand: Record<ProjectId, number>;
  rankedDemand: Record<ProjectId, number>;
};
...
type OptimizeForActiveSetArgs = {
  activeProjectIds: ProjectId[];
  candidates: RankingResponse[];
  lockedRosters: TeamMap;
  projectCount: number;
  seed: number | string;
  mode: MatchingModeId;
};

type OptimizeForActiveSetResult = {
  teams: TeamMap;
  score: number;
};

type SummarizeTeamsArgs = {
  projects: ProjectRecord[];
  responses: RankingResponse[];
  teams: TeamMap;
  assigned: Record<EmailAddress, TeamAssignment>;
  warnings: TeamWarning[];
  mode?: MatchingModeId | string;
};
...
const matchingModeConfig = (mode?: MatchingModeId | string): MatchingModeOption =>
  MATCHING_MODE_OPTIONS.find((option) => option.id === mode) || MATCHING_MODE_OPTIONS[0];

const normalizeEmail = (email: string | null | undefined): EmailAddress =>
...
const tieScore = (value: string, seed: number | string): number => hashSeed(`${seed}:${value}`);

const clamp = (value: number, min: number, max: number): number =>
  Math.max(min, Math.min(max, value));

const activeProjectCountCandidates = (
  projectCount: number,
...
const preferenceUtility = (
  rank: number,
  projectCount: number,
  mode: MatchingModeId | string = DEFAULT_MATCHING_MODE
): number => {
  ...
  if (matchingModeConfig(mode).id === "top1") {
    ...
    return first * 10_000_000 + fullRanking * 1_000 - normalizedRank;
  }
  ...
  return top3 * 1_000_000 + first * 30_000 + top5 * 10_000 + fullRanking * 100 - normalizedRank;
};

const rankSatisfaction = (rank: number, projectCount: number): number => {
...
const subsetsOfProjectIds = (
  projectIds: ProjectId[],
  count: number,
  lockedProjectIds: Set<ProjectId>
): Array<Set<ProjectId>> => {
  ...
  const sets: Array<Set<ProjectId>> = [];

  const walk = (start: number, picked: ProjectId[]) => {
...
const buildDemandMap = (projectIds: ProjectId[], responses: RankingResponse[]): DemandMap => {
  ...
  responses.forEach((response) => {
    response.ranking.forEach((projectId, rank) => {
      if (projectId in rankedDemand)
        rankedDemand[projectId] += Math.max(0, projectIds.length - rank);
    });
...
const activeSetDemandScore = (
  activeSet: Set<ProjectId>,
  { top3Demand, top1Demand, rankedDemand }: DemandMap,
  projectIds: ProjectId[],
  mode: MatchingModeId | string = DEFAULT_MATCHING_MODE
): number => {
  const isTop1Mode = matchingModeConfig(mode).id === "top1";
  return [...activeSet].reduce(
    (total, id) =>
      total +
      (isTop1Mode
...
const minCostMaxFlow = (
  ...
  sink: number,
  targetFlow: number
): { flow: number; cost: number; failed?: true } => {
  let flow = 0;
  let cost = 0;
...
const optimizeForActiveSet = ({
  ...
  seed,
  mode,
}: OptimizeForActiveSetArgs): OptimizeForActiveSetResult | null => {
  const seatPlan: SeatPlanItem[] = [];
  ...
  const viableProjectNeeds = projectNeeds.filter((need): need is ProjectNeed => Boolean(need));
  const totalMin = viableProjectNeeds.reduce((total, need) => total + need.minNeed, 0);
  const totalMax = viableProjectNeeds.reduce((total, need) => total + need.maxNeed, 0);
  ...
  const assignmentEdges: Array<{ edge: FlowEdge; studentIndex: number; seatIndex: number }> = [];
  ...
  candidates.forEach((response, studentIndex) => {
    seatPlan.forEach((seat, seatIndex) => {
      ...
      const utility =
        preferenceUtility(rank, projectCount, mode) +
  ...
  const teams = Object.fromEntries(
    activeProjectIds.map((projectId) => [projectId, [...(lockedRosters[projectId] || [])]])
  ) as TeamMap;
  const filledRequiredSeats = new Set<number>();
...
const summarizeTeams = ({
...
}: SummarizeTeamsArgs): TeamMatchingSummary => {
  ...
  const sizeWarnings = activeTeamIds
    ...
    .map((id): TeamWarning => ({ type: "team-size", projectId: id, size: teams[id].length }));
  ...
  return {
    ...
    unhappyCount: responses.filter(
      (response) => assigned[response.email]?.prefRank >= modeConfig.topChoiceWindow
    ).length,
...
export const buildTeams = ({
  ...
  seed = 0,
  mode = DEFAULT_MATCHING_MODE,
}: BuildTeamsArgs): TeamMatchingSummary => {
  const modeConfig = matchingModeConfig(mode);
  const projectIds = projects.map((project) => project.id);
  const emptyTeams = Object.fromEntries(projects.map((project) => [project.id, []])) as TeamMap;
  ...
  const lockedRosters = Object.fromEntries(projectIds.map((id) => [id, []])) as TeamMap;
  const candidates: RankingResponse[] = [];
  ...
  responses.forEach((response) => {
    ...
    if (honorsProjectId && projectIds.includes(honorsProjectId)) {
      ...
      const lockedStudent: TeamMember = {
        email: response.email,
        name: response.name,
        prefRank,
        locked: true,
    ...
    if (honorsProjectId) {
      warnings.push({
        type: "honors-unassigned",
        email: response.email,
        projectId: honorsProjectId,
      });
    }
  ...
  lockedProjectIds.forEach((projectId) => {
    if (lockedRosters[projectId].length > TEAM_MAX) {
      warnings.push({
        type: "honors-over-capacity",
        projectId,
        size: lockedRosters[projectId].length,
      });
    }
  });

  if (!responses.length) {
    return summarizeTeams({
      projects,
      responses,
      teams: emptyTeams,
      assigned,
  ...
  let best:
    | (OptimizeForActiveSetResult & {
        activeProjectIds: ProjectId[];
        totalScore: number;
      })
    | null = null;

  for (const targetActiveCount of activeProjectCountCandidates(
    projectIds.length,
    responses.length,
    lockedProjectIds.size
  )) {
    const activeSets = (
      modeConfig.id === "top1"
    ...
    activeSets.forEach((activeSet) => {
      ...
      const lockedScore = activeProjectIds
        .flatMap((id) => lockedRosters[id] || [])
        .reduce(
          (total, student) =>
            total + preferenceUtility(student.prefRank, projectCount, modeConfig.id),
          0
        );
  ...
  const teams = Object.fromEntries(
    projects.map((project) => [project.id, winning.teams[project.id] || []])
  ) as TeamMap;
